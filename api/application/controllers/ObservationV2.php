...
class ObservationV2 extends CI_Controller {
...
$headers = $updated_headers;
		if($headers != null && array_key_exists('X-Device-Id', $headers) && array_key_exists('X-Token', $headers)){
			...
			if($user_id!= null && $res != null && $res->userid == $user_id){
				...
				$observationLinks=$this->ObservationModel->getobservationLinks($id);
				$data['observationLinks']=array();
				foreach($observationLinks as $observationLink)
				{
					$ob=$this->ObservationModel->getObservationUser($observationLink->linkid);
					$childrens=$this->ObservationModel->getObservationChildrens($observationLink->linkid);
					$montessoryCount=$this->ObservationModel->getObservationMontessoriCount($observationLink->linkid);
					$eylfCount=$this->ObservationModel->getObservationEylfCount($observationLink->linkid);
					$milestoneCount=$this->ObservationModel->getObservationMilestoneCount($observationLink->linkid);
					$media=$this->ObservationModel->getobservationMedia($observationLink->linkid);
					$data['observationLinks'][]=array('title'=>$ob->title,
													  'status'=>$ob->status,
													  'id'=>$ob->id,
													  'date_added'=>$ob->date_added,
													  'user_name'=>$ob->user_name,
													  'approverName'=>$ob->approverName,
													  'childrens'=>$childrens,
													  'media'=>isset($media[0]->mediaUrl)?$media[0]->mediaUrl:'',
													  'montessoryCount'=>$montessoryCount,
													  'eylfCount'=>$eylfCount,
				...
				$data['reflectionLinks']=array();
				$reflectionLinks=$this->ObservationModel->getReflectionLinks($id);
				foreach($reflectionLinks as $reflectionLink){
					$re=$this->ObservationModel->getReflectionUser($reflectionLink->linkid);
					$childrens=$this->ObservationModel->getReflectionChildrens($reflectionLink->linkid);
					$data['reflectionLinks'][]=array(
						'ol_id' => $reflectionLink->id,
						'title'=>$re->title,
					  	'status'=>$re->status,
						'id'=>$re->id,
						'date_added'=>$re->createdAt,
						'user_name'=>$re->user_name,
						'approverName'=>$re->user_name,
						'about' => $re->about,
						'childrens'=>$childrens
					);
				}

				$qipLinks = $this->ObservationModel->getQipLinks($id);
				foreach ($qipLinks as $qipLink => $qlink) {
					$qipObj = $this->ObservationModel->getQipInfo($qlink->linkid);
					$qlink->name = $qipObj->name;
				}
				$data['qipLinks'] = $qipLinks;

				$programplanLinks = $this->ObservationModel->getProgramPlanLinks($id);
				foreach ($programplanLinks as $progplanLink => $pplink) {
					$ppObj = $this->ObservationModel->getProgramPlanInfo($pplink->linkid);
					$pplink->name = date('d-m-Y',strtotime($ppObj->startdate)) . "/" . date('d-m-Y',strtotime($ppObj->enddate));
				}
				$data['programplanLinks'] = $programplanLinks;
			...
			if($json!= null && $res != null && $res->userid == $json->userid){
				...
				if (empty($obsId)) {
                	...
                	foreach ($obsMonSubArr as $monSubjects => $monsubject) {
                		$getMonSubs = $this->ObservationModel->getMonSub($monsubject);
                		foreach($getMonSubs as $subjects =>$sub){
	                		$getAllMonActArr = $this->ObservationModel->getMonActvts($sub->idSubject);
	                ...
	                $data['montessoriSubjects'] = isset($getMonSubs)?$getMonSubs:NULL;
...
$headers = $updated_headers;	
	...
	public function getPedagogySettings()
	{
		$headers = $this->input->request_headers();
$updated_headers = []; // Temporary array to store modified headers
...
$headers = $updated_headers;	
		if($headers != null && array_key_exists('X-Device-Id', $headers) && array_key_exists('X-Token', $headers)){
			...
			if($json!= null && $res != null && $res->userid == $json->userid){
				...
				$results = $this->ObservationModel->getPedagogySettings($json->centerid);

				foreach ($results as $pedagogyKey => $pedagogyObj) {
					$pedagogySubArr = $this->ObservationModel->getPedagogySubjects($pedagogyObj->id);
					foreach ($pedagogySubArr as $pedagogySubKey => $pedagogySubObj) {
						$pedagogyActArr = $this->ObservationModel->getPedagogyActivities($pedagogySubObj->id);
						foreach ($pedagogyActArr as $pedagogyActKey => $pedagogyActObj) {
							$pedagogySubActArr = $this->ObservationModel->getPedagogySubActivities($pedagogyActObj->id);
							$pedagogyActObj->subactivities = $pedagogySubActArr;
						}
						$pedagogySubObj->activities = $pedagogyActArr;
					}
					$pedagogyObj->subjects = $pedagogySubArr;
				}
