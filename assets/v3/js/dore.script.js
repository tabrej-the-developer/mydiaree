...
$.dore = function (element, options) {
    ...
    function init() {
        ...
        if (typeof Chart !== "undefined") {
            ...
            if (document.getElementById("permissionChart")) {
                function permissionChartInit() {
                    var _totalPermissions = $('.common-checkbox:checkbox').length;
                    var _givenPermissions = $('.common-checkbox:checked').length;
                    var _notGivenPermissions = _totalPermissions - _givenPermissions;
                    var permissionChart = document.getElementById("permissionChart");
                    var myDoughnutChart = new Chart(permissionChart, {
                        plugins: [centerTextPlugin],
                        type: "DoughnutWithShadow",
                        data: {
                            ...
                            datasets: [
                                {
                                    ...
                                    borderWidth: 2,
                                    data: [_givenPermissions, _notGivenPermissions]
                                }
                            ]
                        },
                        draw: function () { },
        ...
        if ($().contextMenu) {
            $.contextMenu({
                ...
                items: {
                    rename: { name: "Rename", className: "iconsminds-rename" },
                    tags: { name: "Tags", className: "iconsminds-tag-3" },
        ...
        if ($('main').length > 0) {
            $('main').append(`
        <button class="btn-floating" id="floating-children-icon"><i class="simple-icon-people"></i></button>
        <div class="floating-table" id="floating-children-data">
            <div class="d-flex justify-content-between">
            <div class="floating-table-title">Child list</div>
            <div id="total-child" style="margin:20px;"></div>
            </div>
          <div class="floating-table-body"></div>
        </div>
      `);
            ...
            $.ajax({
                ...
                .done(function (json) {
                    res = $.parseJSON(json);
                    if (res.Status == "ERROR") {
                        $('#floating-children-icon').hide();
                    } else {
                        $('#floating-children-icon').show();
                    }
                });
                    ...
                    $.ajax({
                        ...
                        .done(function (json) {
                            ...
                            if (res.Status == "ERROR") {
                                ...
                                $('.floating-table-body').empty();
                                $('#total-child').empty().text('Total ' + res.TotalChilds + ' child');
                                $.each(res.Childs, function (index, val) {
                                    //Check Image
                                    if (val.imageUrl == "") {
                                        imageUrl = "https://via.placeholder.com/60x60?text=No+Media";
                                    } else {
                                        imageUrl = "https://mydiaree.com/api/assets/media/" + val.imageUrl;
                                    }

                                    if (val.observation_color == "red") {
                                        warning = `<div class="child-name text-danger" ><strong>` + val.name + `</strong></div>`;
                                    } else {
                                        warning = `<div class="child-name" >` + val.name + `</div>`;
                                    }

                                    if (val.observation_id != null) {
                                        link = `<a class="child-obs-link" href="https://mydiaree.com/observation/view?id=` + val.observation_id + `">View last observation <i class="simple-icon-share-alt"></i></a>`;
                                    } else {
                                        link = `<a class="child-obs-link" href="#">View last observation <i class="simple-icon-share-alt"></i></a>`;
                                    }

                                    $('.floating-table-body').append(`
                    <div class="floating-child-box d-flex p-10">
                    <div class="floating-child-box-image">
                        <img src="` + imageUrl + `" class="img-thumbnail border-0 rounded-circle m-4 align-self-center float-child-img-thumb" alt="">
                    </div>
                    <div class="floating-child-box-metadata">
                        `+ warning + `
                        <div class="child-dob">`+ val.gender + `</div>
                        `+ link + `
                    </div>
                    </div>  
                `);
                    ...
                    var ps = new PerfectScrollbar('.floating-table-body');
                }
            });
        }
