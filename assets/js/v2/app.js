...
var app = {
    ...
    navigation: function(){
        ...
        $(".deskMenuIcon").on("click", function(){
        ...
        $(".app-navigation .openable > a").on("click",function(e){
            ...
            if(!$(".app-container").hasClass("app-sidebar-left-closed")){
                if($(".menu").addClass("current")) {
                    $( ".deskMenuIcon" ).trigger( "click" );
                }
            }
  ...
  /*  customer: function(){
        var customer_id=0;
            $.ajax({
                    url: 'index.php?route=finance/invoice/autocomplete&filter_name=&token=' + getURLVar('token'),
                    dataType: 'json',
    
                    success: function(json) {
                        var msg='';
                        $.each(json,function(key,val){
                            if(customer_id!=0 && customer_id==val['customer_id'])
                            {
                                msg+='<option value="'+val['customer_id']+'" selected="selected">'+val['customer_name']+'</option>';
                            }else{
                                msg+='<option value="'+val['customer_id']+'" >'+val['customer_name']+'</option>';
                            }
  ...
  /* customer: function(){
       if(getURLVar('customer_id'))
        {
            var customer_id=getURLVar('customer_id');
        }else{
            var customer_id=0;
        }
            $.ajax({
              
                    url: 'index.php?route=invoice/customer/autocomplete&filter_name=&token=' + getURLVar('token'),
                    dataType: 'json',
   
                    success: function(json) {
                        var msg='<option id="" value="">--Select Customer--</option>';
                        $.each(json,function(key,val){
                            if(customer_id!=0 && customer_id==val['customer_id'])
                            {
                                msg+='<option id="'+val['customer_email']+'" name="'+val['customer_address']+'" title="'+val['gst_number']+'" value="'+val['customer_id']+'" selected="selected">'+val['customer_name']+'</option>';
                            }else{
                                msg+='<option id="'+val['customer_email']+'" name="'+val['customer_address']+'" title="'+val['gst_number']+'" value="'+val['customer_id']+'" >'+val['customer_name']+'</option>';
                            }
                            
                        });
                        $('#auto_customer_id').html(msg);
                    }
                });
  ...
  */    
    ...
    cost_center: function(){
       if(getURLVar('cost_center_id'))
        {
            var cost_center_id=getURLVar('cost_center_id');
        }else{
            var cost_center_id=0;
        }
            $.ajax({
              
                    url: 'index.php?route=projects/cost_center/autocomplete&filter_name=&token=' + getURLVar('token'),
                    dataType: 'json',
   
                    success: function(json) {
                        
                        var msg='<option id="" value="">--Select cost_center--</option>';
                        $.each(json,function(key,val){
                            if(cost_center_id!=0 && cost_center_id==val['cost_center_id'])
                            {
                                msg+='<option  value="'+val['cost_center_id']+'" selected="selected">'+val['name']+'</option>';
                            }else{
                                msg+='<option  value="'+val['cost_center_id']+'" >'+val['name']+'</option>';
                            }
                            
                        });
                        $('#auto_cost_center_id').html(msg);
                    }
                });
    ...
    */
    ...
    sidetoggle: function(){                
              $("[data-sidebar-minimize]").trigger( "click" );
