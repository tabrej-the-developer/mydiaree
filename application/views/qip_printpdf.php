...
<div id="container">
	...
	<?php foreach($previews as $preview) { $areaid=$preview->areaId; ?>
	<?php if(in_array($preview->areaId,$areas)) { ?>
	<span class="qipsum"><?php echo $preview->areaName; ?></span><br/><br/>
	...
	<span class="qipstr">Strengths:-<?php echo $preview->strength; ?></span>
	  <?php foreach($preview->standards as $standard) { $val= $standard->id; ?>
	  <?php if(isset($sta[$areaid]) && in_array($standard->id,$sta[$areaid])) { ?>
	  ...
	  <?php if(!empty($values)) {  ?>
	  <?php if(!empty($values->$val)) { $v=$values->$val ?>
	  <?php echo $v->val1; ?><br/>
	  <?php } ?>
	  <?php } ?>
	  ...
	  <table class="qiptable">
		...
		<tbody class="tbodypreview">
			<?php 
			if(!empty($preview->planes)){
				foreach($preview->planes as $plan){
			?>
			<tr>
