...
<body id="app-container" class="menu-default show-spinner">
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="col-md-4">
                    <input type="text" id="searchParent" class="form-control" placeholder="Search Parent by Name..." />
                                </div>
                        <div class="filterAll">
                    ...
                    <a class="d-flex"
                        ...
                        <div class="d-flex flex-grow-1 min-width-zero">
                                    ...
                                    <ul class="list-unstyled mt-2">
                                        ...
                                        <li class="ml-3">
                                            ...
                                            <small class="text-muted">
                                                (<?= ucfirst(strtolower($child->relation)); ?>)</small>
                                        </li>
                                        <?php } ?>
                                    </ul>
                                    <?php } else { ?>
                            ...
                            <div style="margin-right: 15px;margin-top: 10px;">
                                ...
                                <?php if ($pobj->status == 'ACTIVE') { ?>
                                <a href="#" class="deactivate-link" data-userid="<?= $pobj->userid; ?>"
    ...
    $(document).ready(function() {
        ...
        $('.deactivate-link').on('click', function(e) {
            ...
            var confirmText = (action === 'deactivate') ? 'Deactivate this user?' :
                'Activate this user?';
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        error: function() {
                            Swal.fire('Error!', 'There was a server error.',
                                'error');
                        }
                    });
                }
...
$(document).ready(function () {
    $('#searchParent').on('keyup', function () {
        let query = $(this).val();
        let centerid = '<?= $centerid ?>'; // Pass PHP variable to JS

        $.ajax({
            url: '<?= base_url("Settings/searchParentsAjax") ?>',
            type: 'POST',
            data: {
                search: query,
                centerid: centerid
            ...
            success: function (response) {
                $('#parentList').html(response); // Replace parent cards
            }
        });
    });
});
